...
(defn load-core-completions-matching-string [startsWithText]
  (let* [
         cljFiles (search-files clojureClrRuntime "*.clj" true)
...
(defn load-completions-in-assembly-matching-string [assembly startsWithText]
  (let* [
         matchingFunctions (filter-function-metas-by-name-starts-with (get-all-public-meta-in-assembly assembly) startsWithText)
        ]
    (map convert-function-meta-to-completion matchingFunctions)
  )
)
...
(defn load-completions-in-clj-file-matching-string [cljFileName startsWithText]
  (let* [
         namespace (convert-filename-to-namespace cljFileName (Path/GetDirectoryName cljFileName))
...
(defn match-parenthesis-with-location [codeWithoutStringsOrCommentsOrEscapedParenthesis]
  (let* [
         pairedParenthesis (match-parenthesis codeWithoutStringsOrCommentsOrEscapedParenthesis)
...
(defn remove-strings-and-comments-and-escaped-parenthesis-from-code [code]
  (let* [
         codeWithoutSingleLineStrings (regex-replace code #"\"(\\\\|\\\"|[^\"\n])*\"" \ )
...
(defn compile-string [code]
  (let* [
         cljFile (Path/ChangeExtension (Path/GetTempFileName) ".clj")
...
(defn select-all-location [code]
  (let [
         newlineCharacters (map :index (indexes-of-characters code [\newline]))
       ]
...
(defn compiler-errors [code]
  (let [
         parseErrors (parse-for-errors code)
